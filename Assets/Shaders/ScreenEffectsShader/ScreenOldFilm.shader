Shader "ShaderTutorial/ScreenOldFilm"
{
	Properties  
    { 
    ...
    SubShader
    {
        Pass
        {
            ...
            fixed4 frag (v2f_img i) : COLOR
            {
                ...
                //Get pixels from vignette texture
                ...
                // Get luminosity values from the render texture using YIQ values
                fixed lum = dot(fixed3(0.299, 0.587, 0.114), renderTex.rgb);
                //Add the constant colour to the lum values
                fixed4 finalColor = lum + lerp(_SepiaColor, _SepiaColor + fixed4(0.1f, 0.1f, 0.1f, 1.0f), _RandomValue);
                finalColor = pow(finalColor, _Contrast);

                //Create a constant white colour to adjust opacity of effects
                fixed3 constantWhite = fixed3(1,1,1);
                //Composite different layers of effects
